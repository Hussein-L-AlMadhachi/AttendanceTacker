...
export function LoginPage() {
    ...
    const navigateRole = useGetRoleFirstPageURL({
        "admin": "/admin/students",
        "teacher": "/teacher/attendance",
        "superadmin": "/superadmin/students"
    })
    ...
    async function login() {
        ...
        try {
            ...
            document.cookie = `auth-role=${data.role}; max-age=86400; path=/`;

            const canResumeQrScan = nextPath?.startsWith("/a/qr-attend")
                && (data.role === "teacher" || data.role === "superadmin");

            if (canResumeQrScan && nextPath) {
            ...
            if (message.includes("account_locked")) {
                setErrorMsg("تم قفل الحساب بعد 6 محاولات دخول فاشلة. تواصل مع المسؤول.");
            } else {
