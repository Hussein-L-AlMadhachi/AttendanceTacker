...
function Options({ onAddClick, onImportSuccess }: OptionsProps) {
    ...
    return <>
        ...
        <Modal isOpen={isStartYearModalOpen} className="w-full flex flex-col justify-center max-w-lg">
            <h3 className="font-bold text-lg mb-4 text-center"> تأكيد الادخال</h3>
...
function MainContent(): JSX.Element {
    ...
    const fetchData = () => {
        superAdminRPC.fetchStudents().then((data) => setData(data));
    };
    ...
    const handleUpdateStudent = async (id: number, data: StudentUpdateData) => {
        await superAdminRPC.updateStudent(id, { student_name: data.student_name });
        fetchData();
    };
